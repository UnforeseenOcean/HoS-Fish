...
function SideView(curObj, enc_userid, targetNick, rolecode, isAdmin, isGuest, isMine) {
	...
	if (isGuest == "GUEST") {
		createMenuItem("blog", "블로그 가기", "goBlog('" + grpid + "', '" + enc_userid + "');");
	} else {
		createMenuItem("member", "회원정보", "goProfile('" + grpid + "', '" + enc_userid + "')");
		createMenuItem("article", "작성글보기", "goArticle('" + enc_userid + "','" + grpid + "', '" + this.item + "', '" + targetNick + "', '" + isAdmin + "', '" + isMine + "')");
		createMenuItem("blog", "블로그 가기", "goBlog('" + grpid + "', '" + enc_userid + "');");
...
function goProfile(grpid, enc_userid) {
	var hostWithProtocol = 'http://' + document.location.host;
	if (document.location.host == 'cafe.daum.net') // 실서버
		hostWithProtocol = 'https://secure.cafe.daum.net';

	var realUrl = hostWithProtocol + '/_c21_/member_profile?grpid='+grpid+'&userid='+enc_userid+'&enc_userid='+enc_userid;
	window.open(realUrl, 'DaumCafeMemberProfileWindow', 'width=382,height=500,resizable=no,scrollbars=no');
}
function goBlog(grpid, enc_userid) {
	var realUrl = getCafeUrl('/_c21_/sideview_hdn?name=blog&grpid='+grpid+'&userid='+enc_userid+'&enc_userid='+enc_userid);
	window.open(realUrl, 'DaumCafeGoBlog', 'scrollbars=yes, resizable=yes');
}

function sendEmail(grpid, enc_userid) {
	var realUrl = 'http://cafe.daum.net/_c21_/mailto_popup?grpid='+grpid+'&userid='+enc_userid+'&euserid='+enc_userid;
	window.open(realUrl, 'DaumCafeSendMail', 'width=660,height=470,resizable=no,scrollbars=yes');
}
...
function sendWebMemo(grpid, enc_userid, isMine) {
	...
	var realUrl = "http://msgbox.cafe.daum.net/client/chat?grpid=" + grpid + "&targetUserid=" + enc_userid;
	window.open(realUrl, 'DaumCafeMsgbox_' + target, 'width=360,height=530,scrollbars=yes').focus();
}


function goArticle(enc_userid, grpid, item, targetNick, isAdmin, isMine) {
	...
	if (isMine == "true") {
		...
		url = getCafeUrl('/_c21_/member_article_cafesearch?grpid=' + grpid + '&item=writer&nickname=' + Base64.encodeURI(targetNick) + '&nickname_enc=base64&enc_userid=' + (enc_userid || ""));
	}
